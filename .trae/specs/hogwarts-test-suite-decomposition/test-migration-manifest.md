...
| dead Legacy Pacing normalizer/validator body in `domain/pacing-validation.js` | delete two unexported zero-caller functions and their dead imports | `causal-pacing-contract.js` current normalizer/validator; retained `buildTemporaryActorPromotionPolicy` and the public causal normalizer/validator re-exports | repository search found no caller for the two Legacy bodies; current callers still import the promotion policy and public normalizer/validator API from this module | file reduced to 49 active lines; focused Pacing/Cast and import-boundary tests pass |
