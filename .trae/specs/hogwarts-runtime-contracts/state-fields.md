...
| `spellbook.version` | 英语-only Spellbook V3 | Spell V3 migration/reducer | resolver、check、Prompt、UI | custom 只存 `nameEn/effectEn`；静态中文进入 locale resource；G3 观察到刷新补扫新 message 时会更新 `lastScannedMessageId` 并提交 `lifecycle_migration` revision，因此当前 `state_settled` 尚非最终持久边界 |
...
| `extra.hogwartsMud.segments[].textEn` | 玩家已付费获得并可见的英语 narration/dialogue | Performer/Opening workflow | render、locale queue；仅带已验证 transaction 的 scene turn 可进入 observer/archive/Knowledge 权威 | 与 `rawText` 互斥；Turn 下游失败时正文行保留，但缺少 `turnTransaction` 的 scene turn 不是 State/Prompt/Knowledge 权威 |
...
| `MODEL_TASK_REGISTRY.local_dynamic_identity_observer.ledgerScope` | `server_ephemeral`，Phase 2 动态伤势观察候选端口 | model task registry/local Identity endpoint | 服务端诊断、既有即时回合伤势结算 | 不写 `modelTaskRuntime`，不增加 `stateRevision`；仅有结构化 Check 目标才路由，端口失败只省略伤势提案，不回退 Regex |
...
| 字段路径 | 真实语义 | 唯一写入者 | 主要读取者 | 兼容/诊断 |
| --- | --- | --- | --- | --- |
| `LowTierContextV1` | 普通低档 Performer 的唯一 User Payload；顶层严格为 `playerTurn/sceneFacts/actorCards/actionOpportunities/memoryActivations/prohibitions` 六字段 | `projectLowTierContextV1()` | `turn-performance.js` 唯一 Scene Performance 请求 | 总量不超过 50 KiB；单 Actor Card 不超过 4 KiB；不得注入 raw Actor Library、完整 Social Graph、完整 Identity、人物记忆正文账本或全量历史；非法响应直接失败，无 repair |
| `LowTierContextV1.sceneFacts.authoritySnapshot` | 当前 revision 的 Scene、Actor、Item、Material、Room 与 open fact 权威快照 | `buildNarrativeAuthoritySnapshot()` 经 `projectLowTierContextV1()` | matching low-tier Performer、正文一致性 diagnostics | 只作为六字段结构的内部事实投影，不持久化；Material/Room/Item sibling 已删除，当前请求只读这一份权威 |
...
| `LowTierContextV1.playerTurn.movementResolution` | 当前玩家标签移动的已验证 `moved/already_there/failed` 结果与确定性 `movementOutcomeFactEn` | movement precondition settlement 经 Low projector | Low Performer、正文一致性 fold | protected；失败必须包含 attempt/reason/remaining room；不得与 sibling 位置投影重复；模型不能改写结果 |
...
| `buildMapAuthorityContext()` | World Expansion 的 preset catalog、active topology 与 runtime diff 单份投影 | `maps.js` | Map Expansion | Tina 序列化为 39,045 字符；Map Expansion 已删除 raw `currentMapState` sibling；Pacing/Transition/Calendar 分别使用 causal/current-scene/location-directory 投影 |
| Role repair envelope | validation error、bounded invalid output 与一份 canonical `originalRequest` | retained workflow repair builder | 对应 retained High/Medium role model | 所有 active Low 与 Scene Chronicle 非法不 retry；不再发送 `requiredSchema` 完整 System |
| Local structured model request | Ollama pre/post/Appraisal/translation 的 System + User 与 transport JSON Schema | `callStructuredModel()` | 本地 semantic/Appraisal/translation model | JSON Schema 只通过 `format` 发送一次；Revision 19 pre 为 `temporal/check/calendarCommitment/movementIntent`，`progression` 已删除，已知中文 prefixed-date Calendar 漏判登记为 TODO；post 仅提既有 `materialEvents/actorUpdates/inventoryObservationRequired/perception/temporalClaims`，不判断 Event |
| Dynamic Inventory observation | post core 的瞬态 `inventoryObservationRequired` 路由后的现有 Item proposal；不是 State | `observeDynamicInventory()` + `INVENTORY_OBSERVATION_RESULT_DESCRIPTOR` | browser adapter、Inventory evidence/stable-ID/transition guards、Item Reducer | `false` 时 0 调用；`true` 时精确 1 次 4B、`num_ctx=4096`、串行并卸载；未知展示名 owner/holder 直接拒绝，禁止改写成 `player`；失败或 guard 拒绝只省略 Item proposal，不回退 Regex、不重试、不影响正文 |
| Native Hogwarts System projection | host native generation 的 world/campaign/player 与 13-key compact committed Scene State | `buildMandatorySceneState()` + `applySystemPrompt()` | SillyTavern native System injection | Tina 19,724 chars；`playerPosition` 一份；Scene 无 itemStates；Actor Core/Runtime 合并为 actorCards；Items 为 current visible compact list；不含私有 knowledge、完整地图、Memory 或 Social Graph |
...
| `postTurnSemanticProvider` | VCON-013 的执行源配置，只允许 `low` 或 `local`；`low` 复用 `modelSlots.low` 完整 Connection Profile，`local` 使用现有 Ollama post 模型 | settings profile controller、initial world、lifecycle normalization | post semantic adapter、models settings UI | 缺失/非法旧档原子归一为 `low`；同时写 extension setting 与 active timeline；不是 gameplay semantic State、modelTaskRuntime 或模型 proposal writer；选中源失败只省略 post proposal，不切换另一源 |
...
| `globalChronicle.version/entries[]` | 每个已封存 Scene 一条 `sceneId/endedClock/summaryEn` 语义摘要；按 archive 顺序 append-only | Scene Transition reducer、一次性无模型迁移 | Knowledge Scene projector | revision 4 已实现；不向前端开放，不把整表直接注入 Prompt；Daily 退役后无 Daily reader |
| `stateRevision` | 当前时间线非负、单调递增 revision | guarded save commit | 所有 Hogwarts metadata/chat save、stale boundary guards | rollback 也只能增加；chat-only 不增加；guard 可接受 revisionHistory 证明为连续 `model_task_runtime`-only 的增量 |
| `revisionHistory[]` | 最近 48 次世界/运行账本变化的 source/domain 与 Item/Identity before/after | save revision commit | diagnostics、stale boundary guards、未来 event proposal | scheduler ledger 保存使用 `source/domain=model_task_runtime` 且不得混入 `world`；不保存完整快照、聊天正文、secret 或 prompt |
| `clock` | 当前权威世界时间 | turn/transition reducers | 全部导演、UI | 正文不能反写 |
| `knowledgeBase` | 当前时间线 Knowledge timeline ID、API/index/projector version、projection fingerprint、同步状态、record hash 与 bounded diagnostics | knowledge adapter/projector | lifecycle、inspector、debug | `projectionFingerprint` 只由排序后的 record ID + content checksum 决定；metadata-only revision 不触发重复 embedding；上次 Qdrant `degraded` 时内容未变快路径必须重做 manifest 对账直到恢复；JSON/Qdrant/Vectra 不是 State 权威 |
| `knowledgeBase.retrievalDiagnostics` | Planner、实际 backend、降级状态、selected/source paths、capsule ID、local call count | knowledge adapter | turn diagnostics、debug | 有界且不保存完整私有 Prompt/secret；API/exact retrieval 失败在 Low 前显式终止当前回合，健康零匹配才返回空结果 |
| `modelTaskRuntime.version/byTaskId` | 模型任务调用账本：attempt/success/failure/deferred/skip 计数、最后 trigger/emitter/actionId/revision/turn/Scene 与 next eligibility | deterministic model event scheduler | scheduler、跨事件 player-action quota、diagnostics、turn commit ledger merge | Revision 16 增加 `local_event_boundary_observer` world-ledger row；未发生的 revision/turn/eligibility 保持 `null` 且 normalize 必须幂等；不保存 Prompt、response、secret 或 State snapshot；每次持久化后 outcome writer 必须重新获取 live State |
| `modelTaskRuntime.byTaskId.local_event_boundary_observer` | 每 10 committed turns 后台 Event 判断 checkpoint 账本 | model event scheduler | background Event scheduler、diagnostics | 仅 turns `10/20/30/...`；attempt 即锁定该 checkpoint，失败无重试，下次仅下一 checkpoint |
| `memoryDirector.pendingEventBoundary` | 已接受的长互动 Event 呼吸口；供既有 next-intent/Memory/Social 下游消费 | Revision 16 background Event boundary reducer | next-intent、Memory/Social boundary consumers | immediate paid/post turn 不再写；`ended=false`、失败或 stale 结果不写；不等于 Scene close |
...
| `calendar.entries[]` | 玩家可见的精确 schedule 集合 | Medium Calendar proposal Reducer、V1 migration、clock settlement、Scene archive linker | Medium/Scene claim projections、Calendar UI | `entryType` 固定 event；V1 High event 可 grandfather；Canon 等只作普通 tag |
...
| `map.customLocalMaps[].worldAnchorId` | custom map 对应的合法 preset world node 锚点；无合法锚点时为空 | Opening/Interior reducer、一次性 mount migration | map UI、Calendar/Scene location projection | 不得用 custom parent map ID 伪造 world node；generated child 从 parent 链派生 |
| `map.customLocalMaps[].mount.parentMapId/parentRoomId` | generated/preset child interior 的唯一精确父房间挂载 authority | Interior reducer、一次性 mount migration | derived mount index、movement bridge、map/Calendar/Scene destination readers | 两字段同时存在；同一父房间最多一个 child；UI 按父房间层级展示 |
| `map.customLocalMaps[].parentWorldNodeId/parentMapId/parentRoomId/sourceContainerKey` | 已删除的 legacy generated-child aliases | 无生产写入者 | 仅一次性 mount migration 读取后删除 | 不双读、不回退；冲突、缺父图/房间或重复 mount 时原子失败 |
| `map.interiorMapBindings` | 已删除的 legacy 持久化反向索引 | 无生产写入者 | 仅一次性 mount migration 交叉校验后删除 | runtime index 由 `customLocalMaps[].mount` 即时派生 |
| `map.interiorMapGeneration.bindingKey` | 最近 Interior Cartographer 尝试的有界诊断键 | Interior workflow | 失败抑制、诊断 UI | 不用于 re-entry、movement 或层级 authority |
| `spatial.player` | 玩家结构化 mapId/roomId | movement/transition reducers | pathfinding、visibility | 必须与当前 map/scene 对齐 |
| `spatial.lastMovement` | 最近一次实际完成的已提交移动 | movement reducer | retry、migration | `failed/already_there` 必须保留旧值；不是最近 movement attempt 或 prose 推断缓存 |
...
| `actors[]` | `ActorRuntimeV1` 运行态；顶层严格为 `id/mapId/roomId/locationKnown/present/lifeStatus/lifeStatusPermanent/lifeStatusDetailEn/lifeStatusSinceClock/currentActivityEn/currentIntentEn/currentGoalEn/temporary` | Actor Context runtime、turn/transition/spatial reducers、一次性 cutover | Authority Snapshot、Dossier current、presence、Scene Transition validator | Revision 16 增加明确 unknown-location 状态；不复制 Actor Core、Identity、关系、Appraisal、Schema 或记忆 |
| `actors[].locationKnown` | 当前 `mapId/roomId` 是否为权威已知位置 | Actor/Presence reducer、Scene Transition reducer、Actor Context V1→V2 一次性 cutover | localPresence、spatial、Authority Snapshot、Dossier | 旧 V1 Runtime 原子派生：有 map/room 为 `true`，缺位置为 `false` 并强制 `present=false`；运行时 `true` 要求合法 map/room，`false` 要求两者为空且 Actor 不在 active/local presence；只有后续合法位置证据可恢复 |
...
| `dailyDirector` | 已退役旧模型计划字段 | 无生产写入者 | lifecycle 一次性删除 | time policy 已确定化；Actor intent/Appraisal/Scene/Clue 分别由既有 authority 负责；不兼容读取、不回退 |
| `pacingDirector` | causal-collapse 回合前评估、每 Scene 调用门禁与下一低档回合必须消费的 pending causal beat | Pacing reducer/workflow | Low Performer、turn validation/consume、transition reset | 仅 `causal_collapse_opportunity` 可触发，每 Scene 最多一次；禁止 actor admission、roster/filler/mishap/generic initiative；与 Social 共用 action medium quota |
| `memoryDirector` | event-boundary Social/Appraisal consolidation 状态 | memory workflow/reducer | Social Director guard、boundary idempotency | 正常 review 至少间隔 10 回合；catch-up 每次最多一批；quota deferred 保持 pending 且不丢边界；stale guard 忽略已证明的 ledger-only revision，但 timeline/turn/Scene/boundary 或其他 revision 变化仍拒绝 |
...
| `extra.hogwartsMud.localAdjudication.result` | 一次 pre-turn `1.7B` 的瞬态 `temporal/check/calendarCommitment/movementIntent` 结构化提案快照 | local semantic adapter + turn workflow | 当前回合预算、Check、Calendar route、跟随移动 settlement、retry | Revision 19 接受当前 pre production candidate；Revision 18 已删除 `progression`；已知中文日期前置 Calendar 漏判只登记 TODO，失败时 Calendar 不写；`movementIntent` 仅选 supplied guide/room ID 与 evidence；model/Schema 失败对明确移动标签生成 deterministic failed outcome；不 retry、不用 Regex 补猜 |
| `extra.hogwartsMud.movementPreflight` | 当前尚未成功提交 turnTransaction 的有界移动预结算与 retry 诊断 | turn workflow movement precondition settlement | 当前 paid Prompt、同一玩家消息 retry | 临时字段；成功提交后删除；不是 committed movement authority，不进入后续 Prompt/Knowledge |
...
| `extra.hogwartsMud.turnTransaction.movementOutcome` | 当前标签移动的唯一 committed `MovementOutcomeV1`：moved/already_there/failed、稳定原因、from/to/remaining、guide/evidence、分钟 | movement precondition settlement + turn transaction writer | Low handoff audit、retry、spatial history、debug | 只有 moved 可更新位置/`spatial.lastMovement`；failed/already 为 15 分钟且保留位置；历史 user-message `movement` 只读、不再新增 |
