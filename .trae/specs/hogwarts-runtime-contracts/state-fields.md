...
| Local structured model request | Ollama pre/post/inventory/Appraisal/translation 的 System + User 与 transport JSON Schema | `callStructuredModel()` | 本地 semantic/inventory/translation model | JSON Schema 只通过 `format` 发送一次；post-turn User 只含 `narrativeSegments`，服务端按需派生 text；Tina post-turn 为 10,385 chars / 3,462 estimated tokens |
...
| `MODEL_TASK_REGISTRY` / model event definitions | 全部活跃生成任务的静态职责、档位、触发事件、阶段、预算、Schema、validator、reducer、重试与失败策略 | `domain/model-task-registry.js` | scheduler、build-only inventory、diagnostics | 非 State；登记 20 个历史任务，runtime 仅 16 active，Foundation/Opening Plan/Opening Dialogue/Daily 为 retired 且无 caller |
...
| `modelTaskRuntime.version/byTaskId` | 模型任务调用账本：attempt/success/failure/deferred/skip 计数、最后 trigger/emitter/actionId/revision/turn/Scene 与 next eligibility | deterministic model event scheduler | scheduler、跨事件 player-action quota、diagnostics、turn commit ledger merge | 仅含 16 active task rows；未发生的 revision/turn/eligibility 保持 `null` 且 normalize 必须幂等；不保存 Prompt、response、secret 或 State snapshot；同 actionId 最多一个同步 medium task；每次持久化后 outcome writer 必须重新获取 live State，禁止写入已替换对象 |
