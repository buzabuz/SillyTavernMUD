...
| `session.localizationVisiblePending/localizationPriorityKeys/localizationLastErrorCode` | 当前页面可见翻译等待、优先字段和有界错误状态 | translation controller、localized render projection | locale/status UI | 不写 JSONL；不保存译文 |
| `jobRegistry.localizationActiveBatch` | 当前唯一正在执行的翻译 batch keys；`null` 表示无 active batch | idle localization scheduler | scheduler gate、current-turn progress UI | 不写 JSONL；已保存当前消息的 selected-provider P0 batch 与 selected post 独立启动；本地任务共享既有 FIFO |
...
| Local structured model request | Ollama pre/post/Appraisal/translation 的 System + User 与 transport JSON Schema | `callStructuredModel()` | 本地 semantic/Appraisal/translation model | JSON Schema 只通过 `format` 发送一次；pre 为 `temporal/check/calendarCommitment`，不再判断 movement；post 对触发的 `movementPreflight` 仅提出 evidence-bound `playerMovement`，不判断 Event；已知中文 prefixed-date Calendar 漏判登记为 TODO |
| Dynamic Inventory observation | post core 的瞬态 `inventoryObservationRequired` 路由后的现有 Item proposal；不是 State | `observeDynamicInventory()` + `INVENTORY_OBSERVATION_RESULT_DESCRIPTOR` | browser adapter、Inventory evidence/stable-ID/transition guards、Item Reducer | standalone Inventory 使用 9,000 字符 gate；shared Dynamic Turn 仅在最终合成请求使用 20,000 字符 gate。`false` 时 0 调用；`true` 时精确 1 次 4B、`num_ctx=4096`、串行并卸载；未知展示名 owner/holder 直接拒绝，禁止改写成 `player`；失败或 guard 拒绝只省略 Item proposal，不回退 Regex、不重试、不影响正文 |
...
| `knowledgeBase` | 当前时间线 Knowledge timeline ID、API/index/projector version、projection fingerprint、同步状态、record hash 与 bounded diagnostics | knowledge adapter/projector | lifecycle、inspector、debug | `projectionFingerprint` 只由排序后的 record ID + content checksum 决定；metadata-only revision 不触发重复 embedding；Qdrant 仅在 JSON exact 成功后后台 manifest repair，内容未变但 degraded 时也必须继续对账直到恢复；Node 启动时只从 JSON exact 记录文件恢复未完成派生 repair；health 在同 fingerprint repair completed 后清除旧 degraded；`diagnostics.qdrantRepair` 仅含白名单 operation/method/path、status、transport code 与 queue/progress 摘要；JSON/Qdrant/Vectra 不是 State 权威 |
...
| `turn` | 回合计数、状态、错误、最近耗时 | turn reducer/workflow | recovery、UI、directors | `post_unsettled` 表示已保存的 paid Scene 正等待人工 Post-only settlement；无论是否触发移动，它都不提交世界回合，锁定新动作，reload 不得自动调用模型；`resolving` 也不授权 reload 自动调用模型 |
...
| `spatial.player` | 玩家结构化 mapId/roomId | Movement Reducer、transition reducer | pathfinding、visibility | 必须与当前 map/scene 对齐；Preflight、paid narration、Post candidate、reload 与 retry 均不得直接写入 |
