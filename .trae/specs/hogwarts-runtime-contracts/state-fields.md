...
| 字段路径 | 真实语义 | 唯一写入者 | 主要读取者 | 兼容/诊断 |
| --- | --- | --- | --- | --- |
| `scene` | 当前场景 ID、摘要、起点、room、next intent | opening/transition/scene reducers | turn、UI、archive | `startedMessageId` 绑定聊天范围 |
...
| `actorLibrary[].sharedMemories` | 按 core/recent/everyday 分层的人物特定记忆 | turn/transition/memory reducers | actor capsule、inspector | 禁止从 closure summary 或 roster 批量生成 |
...
| `eventKnowledge[]` | 事件 participant/witness/perception 事实 | event knowledge reducer | social、memory、knowledge | 不直接修改关系 |
