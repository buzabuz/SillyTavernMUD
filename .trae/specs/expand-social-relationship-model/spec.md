...
- 未命中当前关系筛选、但两个端点仍符合人物范围/身份/搜索条件的 player-known 边以 5% 不透明度保留为不可交互背景；它们不进入当前可见关系计数、详情或文本回退。
- 点击节点高亮一跳关系并打开详情侧栏。
...
- **WHEN** 玩家筛选负面关系，而 Lavender→Player 是正面关系
- **THEN** Lavender→Player 不进入当前可见关系计数、详情或文本回退
- **AND** 若 Lavender 与 Player 节点仍符合其他人物筛选，该边以 5% 不透明度留作不可交互背景
- **AND** 若任一端点被范围、身份或搜索条件移除，该边完全隐藏
