# Phase 1 Independent Acceptance Task - Attempt 3
...
decrease, governance history must not be silently rewritten, and a retired
semantic fingerprint must never return. Phase 1 must make zero production
behavior and zero model-call changes. Phase 2 and Phase 3 must remain
unimplemented.
```
...
3. Inspect the complete row registry and production consequences, not only
   aggregate counts or named samples.
4. Exercise positive and adversarial enforcement behavior, including
   persisted-row drift, governance-file mutation, unregistered deletion,
   owner/category mutation and retired-ID resurrection.
