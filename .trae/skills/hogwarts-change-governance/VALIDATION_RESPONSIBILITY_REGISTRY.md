...
| `VCON-013` | `post_turn_semantic_proposal`<br>Low complete core/Item/direct-injury proposal; Local core Material/Item-route/Actor/presence/perception/temporal/player-movement proposal | `implementation_in_progress` | post_turn_semantic_proposal | `provider_aware_strict_descriptor` | post_turn_semantic_provider | none | `turn_post_observation_settlement` + `movement_reducer` | retry=forbidden; provider fallback=forbidden; semantic regex=forbidden; every selected Post no-fit/provider/schema/guard failure preserves paid narration in post_unsettled and permits only explicit Post-only retry or discard | Low: exactly 1 selected Post / 0 Local/4B; Local: 1 core plus 0..1 serial shared 4B / selected Post starts independently from VCON-016 / 1 loaded |
