...
| SETTINGS-002 | Model/API/provider IDs and Custom Endpoint URL | technical setting/service response or user-entered configuration | `ui/settings-profile-controller.js` | `ALLOW` | exact model/API/provider identifier or URL | Allowed only in configuration; Custom Endpoint suffix warning is static |
...
| SETTINGS-004 | Profile, connection, and Custom Endpoint advisory prose | operation or editor validation state | `ui/settings-profile-controller.js`, `ui/bindings.js` | `STATIC` | static locale keys | Endpoint advisory must render localized red text; raw backend error remains **FAIL** |
...
| GEN-006 | Current-turn translation and State-settlement progress | `session.liveSceneStream.phase`, `session.localizationVisiblePending`, `jobRegistry.localizationActiveBatch` | `ui/message-renderer.js::renderLiveSceneStream` | `STATIC` | `ui.message.stream.phase.*` and `ui.message.stream.detail.*`; translating is visible only for a real current-turn P0 batch, committing only for post/State settlement | Current-turn parallelization implementation in progress; desktop/mobile browser acceptance pending |
| GEN-007 | Preserved Scene Post-settlement recovery card and composer lock | `turn.status === post_unsettled` plus bounded pending-settlement status and compaction section codes | `ui/story-renderer.js`, `ui/turn-controller.js` | `STATIC` | `ui.story.post_unsettled.*`; only finite static state, approved compaction section labels and command labels are rendered, never a raw model/provider error | PATCH: selected Post capacity and settlement safety; browser evidence pending |
| NFP-UI-001 | Message-local new speaker name or unresolved label | `message.extra.hogwartsMud.speakers[]` matched to segment reference, never formal Actor admission | `ui/message-renderer.js` | `TranslationTable` or `STATIC` unresolved | `recordKind=message_speaker`, `recordId=message:<messageId>:speaker:<id>`, `fieldPath=displayNameEn`; static `ui.message.unresolved_speaker` until resolved | PATCH: approved narrative-first PRD; rendered verification pending |
| NFP-UI-002 | Recovery failed-family selection and remaining supplement | `pendingPostSettlement.recovery.groups[]/supplement.status`, checkbox selection session-only | `ui/story-renderer.js` | `STATIC` plus integer count | `ui.story.post_family.*`, `ui.story.post_unsettled.supplement/defaults/spent`; no raw IDs/provider errors; one combined request | PATCH: approved narrative-first PRD; rendered verification pending |
| NFP-UI-003 | Initial storage failure, session-visible preserved narrative and storage-only retry | `jobRegistry.unsavedPostNarrative`, scoped to exact context chat reference | `ui/story-renderer.js` and existing message renderer | `STATIC` status plus existing `MSG-004/005` body route | `ui.story.post_unsettled.unsaved/save_only`; explicitly not durable, no committed transaction or new-action authority, no raw storage error; save-only command never requests a model | PATCH: initial-save real-port workflow and desktop/narrow isolated button transition pass; full application pending |
| GEN-008 | Foreground Hogwarts model task activity and elapsed wait | ephemeral `session.foregroundModelActivity.taskId/phase/startedAt` from active `blocking=true` role-task lifecycle | existing Setup, Story, Transition, Calendar and Map loading surfaces | `STATIC` | finite task/phase labels plus locale-formatted elapsed seconds; raw Prompt/content/reasoning/error never reaches DOM | Implemented; real Post runtime plus desktop/mobile acceptance passed |
...
| CAL-036 | Calendar validation/runtime error | validator/model error | Calendar Controller | `STATIC` | known validation, language-skip, and generic runtime-error locale keys; raw detail console-only | Language-skip focused UI test passed; Calendar opened normally in direct browser inspection |
